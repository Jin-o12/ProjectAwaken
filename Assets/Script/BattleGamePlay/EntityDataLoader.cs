...
public static class EntityDataLoader
{
    ...
    public static void LoadEntityStatus()
    {
        ...
        if (jsonFile == null)
        {
            Debug.LogError("EntityDataLoader: entityData.json 파일을 찾을 수 없습니다!");
            entityDataMap = new Dictionary<EntityCord, EntityStatus>();
            return;
        }
        string wrapped = "{\"entityData\":" + jsonFile.text + "}";
        ...
        Debug.Log($"EntityDataLoader: 카드 {entityDataMap.Count}장 로딩됨");
    }
}
